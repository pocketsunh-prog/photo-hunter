...
async function main() {
  ...
  // editing this test, but a full eight-volume game is expected here.
  check('at least 80 chapters are seeded', chapterTotal >= 80, `chapters=${chapterTotal}`);
  ...
  check(
    ...
    new Set((list.json?.levels ?? []).map((l) => l.collection)).size >= 8,
